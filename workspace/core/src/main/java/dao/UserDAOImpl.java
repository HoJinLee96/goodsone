...
public class UserDAOImpl implements UserDAO {
    ...
    public void addUser(User user) throws SQLException {
        String sql = "INSERT INTO user (user_id, user_password, user_oldpassword, user_name, user_nickname, user_birth, user_phone_agency, user_phone_number, user_email, user_address, created_at, user_status, user_signtype) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection connection = dataSource.getConnection();
            ...
            statement.setString(9, user.getUserEmail());
            statement.setString(10, user.getUserAddress());
            statement.setTimestamp(11, Timestamp.valueOf(user.getCreatedAt()));
            statement.setString(12, user.getUserStatus());
            statement.setString(13, user.getUserSignType());
    ...
    public void updateUser(User user) throws SQLException {
        String sql = "UPDATE user SET user_id = ?, user_password = ?, user_oldpassword = ?, user_name = ?, user_nickname = ?, user_birth = ?, user_phone_agency = ?, user_phone_number = ?, user_email = ?, user_address = ?, updated_at = ?, user_status = ?, user_signtype = ? WHERE user_seq = ?";
        try (Connection connection = dataSource.getConnection();
             PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setString(1, user.getUserId());
            statement.setString(2, user.getUserPassword());
            ...
            statement.setString(9, user.getUserEmail());
            statement.setString(10, user.getUserAddress());
            statement.setTimestamp(11, user.getUpdatedAt() != null ? Timestamp.valueOf(user.getUpdatedAt()) : null);
            statement.setString(12, user.getUserStatus());
            statement.setString(13, user.getUserSignType());
            statement.setInt(14, user.getUserSeq());
